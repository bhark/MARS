...
pub(crate) fn render(skel: &Skeleton) -> String {
    ...
    out.push_str("# Generated by mars-import-mapfile — experimental scaffold, not a production config.\n");
    out.push_str("# Coverage: service / layer name and scale window only. Styles, classes,\n");
    ...
    if skel.layers.is_empty() {
        ...
        for layer in &skel.layers {
            ...
            let _ = writeln!(
                out,
                "    # scaffold: hand-tune layer {} — type, sources, classes are not translated",
                layer.name
            );
        }
    }
