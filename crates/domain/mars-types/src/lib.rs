...
mod tests {
    ...
    fn manifest_pointer_rejects_garbage() {
        assert_eq!(
            validate_manifest_pointer(""),
            Err(ManifestPointerError::Empty)
        );
        assert_eq!(
            validate_manifest_pointer("v"),
            Err(ManifestPointerError::BadShape)
        );
        assert_eq!(
            validate_manifest_pointer("vAA"),
            Err(ManifestPointerError::BadShape)
        );
        assert_eq!(
            validate_manifest_pointer("1"),
            Err(ManifestPointerError::BadShape)
        );
        assert_eq!(
            validate_manifest_pointer("v1.0"),
            Err(ManifestPointerError::BadShape)
        );
        assert_eq!(
        ...
        assert_eq!(
            validate_manifest_pointer(&big),
            Err(ManifestPointerError::TooLong)
        );
    }
