...
//! compiler is the primary enforcer but a sidecar carried over from an
//! older epoch could still surface the mismatch on a hot replica.
...
const ORPHAN_SLOT: u32 = 9_999_999;
...
async fn label_survival_follow_geometry_drops_orphan_labels() {
    let opts = FixtureOptions {
        ...
        orphan_label_feature_idxs: vec![ORPHAN_SLOT],
    ...
    assert!(
        !orphan_emitted(&log),
        "FollowGeometry must drop labels whose feature_idx is out of range for the page geometry"
    );
}
